			...
			<div class="interface-wrapper">
				...
				<header class="admin-header">
					<div class="admin-header-wrapper">
						...
						<?php if(isset($_SESSION['admin'])): ?>
							<p class="admin-header-name"><?= $_SESSION['admin']->username ?></p>
							<a href="destroySession.php" class="admin-header-action waves waves-light"><i class="fas fa-sign-out-alt"></i></a>
							<?php else: ?>
								<span></span>
								<span></span>
							<?php endif; ?>
					...
					<?php if(!isset($_SESSION['admin'])): ?>
						<!-- LOGIN -->
						<div class="login">
							<form action="loginSession.php" method="POST" class="login-form" autocomplete="off" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="400">
								...
								<?php if(isset($_SESSION['invalid']['username'])): ?>
									<span class="login-form-invalid"><?= $_SESSION['invalid']['username'] ?></span>
								<?php endif; ?>
								<div class="login-form-box">
									<input type="password" name="password" id="password" class="login-form-input" required>
									<label for="password" class="login-form-label">Password</label>
								</div>
								<?php if(isset($_SESSION['invalid']['password'])): ?>
									<span class="login-form-invalid"><?= $_SESSION['invalid']['password'] ?></span>
								<?php endif; ?>
								<button type="submit" class="login-form-btn waves waves-light">Empezemos...</button>
								<?php isset($_SESSION['invalid']) ? $_SESSION['invalid'] = null : null; ?>
							</form>
						</div>
						<?php else: ?>
							<!-- MESSAGES -->
